...
class Maxwell2D(TimeDependentProblem, SpatialProblem):
    """
    Maxwell 2D Dynamic Electric Field Problem with MaterialHandler integration.
    """
    output_variables = ['E_x', 'E_y', 'H_z']
    spatial_domain = CartesianDomain({'x': [-1, 2], 'y': [-1, 1]})
    ...
    def __init__(self, material_handler):
        """
        Initialize Maxwell 2D Problem with a MaterialHandler.
    ...
    def _build_conditions(self):
        """
        Build dynamic conditions for Maxwell2D based on materials and their interfaces.
        ...
        # 添加初始條件
        '''
        conditions['initial_condition'] = Condition(
            location=CartesianDomain({'x': [-1, 2], 'y': [-1, 1], 't': 0}),
            equation=Equation(initial_condition)
        )
        '''
        # 添加固定邊界條件
        conditions['gamma1'] = Condition(
            location=CartesianDomain({'x': -1, 'y': [-1, 1], 't': [0, 1]}),
            equation=FixedValue(1.0)
        )
        conditions['gamma2'] = Condition(
            location=CartesianDomain({'x': 2, 'y': [-1, 1], 't': [0, 1]}),
...
if __name__ == "__main__":
    ...
    problem = Maxwell2D(material_handler=material_handler)
